 ...
 *
 * Armed with EMACS.
 * Time-stamp: <2019-08-19 17:01:58 macan>
 *
...
int main(int argc, char *argv[])
{
    ...
    gk_info(xnet, "XNET Simple UNIT TESTing ...\n");
    gk_info(xnet, "type 0/1 => Server/Client\n");
    gk_info(xnet, "Args: IP PORT RIP RPORT\n");

    value = getenv("type");
    if (value) {
        type = atoi(value);
    } else {
        gk_err(xnet, "Please set the type=?\n");
        return EINVAL;
    }
    ...
    gk_info(xnet, "Use Args: IP=%s, PORT=%d, RIP=%s, RPORT=%d\n",
    ...
    case 1:                     /* client */
        ...
        break;
    default:
        gk_err(xnet, "Invalid TYPE: please use 0/1!\n");
        return EINVAL;
    }
...
/*                      site, GK_TYPE_SEL(TYPE_MDS, target)); */

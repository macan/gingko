 ...
 *
 * Armed with EMACS.
 * Time-stamp: <2019-08-19 16:17:35 macan>
 *
             ...
             char **type, char **node, int *port, int *id)
{
    ...
    if (*ps != PARSER_EXPECT_SITE) {
        gk_err(lib, "invalid parser state, %s need the %s state\n",
    ...
    if (isdigit(str[0])) {
        /* ip address */
        gk_debug(lib, "> get site ip address: %s\n", str);
    } else if (isalpha(str[0])) {
        gk_debug(lib, "> get site host name: %s\n", str);
    } else {
        res = PARSER_FAILED;
        goto out;
    }
...
int conf_parse(char *conf_file, struct conf_site *cs, int *csnr)
{
    ...
    if (!conf_file) {
        conf_file = "./conf/gk.conf";
    }
    fp = fopen(conf_file, "r");
    if (!fp) {
        gk_err(lib, "fopen() file %s failed w/ %s\n", 
    ...
    *csnr = i;
    {
        int j;
        for (j = 0; j < i; j++) {
            gk_info(lib, "> type %s node %s port %d id %d\n",

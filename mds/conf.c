 ...
 *
 * Armed with EMACS.
 * Time-stamp: <2019-09-04 09:29:37 macan>
 *
...
static int __dconf_write(char *str, int fd)
{
    ...
    do {
        ...
        if (bw == -1) {
            gk_err(mds, "send to fd %d failed w/ %s(%d)\n",
...
static void __dconf_cmd_action(struct dconf_req *dcr, int fd)
{
    ...
    gk_verbose(mds, "ACTION on CMD %ld %ld...\n", dcr->cmd, dcr->arg0);
    switch (dcr->cmd) {
    case DCONF_ECHO_CONF:
    {
        ...
        snprintf(str, 1023, "%s %lx Uptime %lds State %s, "
                 ...
                 (GK_IS_MDS(hmo.site_id) ? "MDS Server" : 
                  (GK_IS_CLIENT(hmo.site_id) ? "Client" : "BP")),
                 hmo.site_id,
                    ...
                    (hmo.state == HMO_STATE_PAUSE ? "PAUSE" :
                     (hmo.state == HMO_STATE_RDONLY ? "RDONLY" :
                      (hmo.state == HMO_STATE_OFFLINE ? "OFFLINE" :
                       "Unknown")))))),
                 (hmo.ring_site == 0 ? GK_ROOT(0) : hmo.ring_site), 
                 /* rusage */
    ...
    case DCONF_SET_PROF_INTV:
        if (dcr->arg0 >= 0) {
            gk_info(mds, "Changing Prof Interval to %ld\n", dcr->arg0);
...
static void __dconf_read_and_reply(int fd)
{
    ...
    do {
        ...
        if (bi == -1 && errno != EAGAIN) {
            gk_err(mds, "cmd channel recv failed %d\n", errno);
            return;
        }
        if (bi > 0)
            bl += bi;
...
static void *mds_dconf_thread_main(void *arg)
{
    ...
    while (!hmo.dconf_thread_stop) {
        ...
        if (err == -1 && errno == EINTR) {
            ...
            gk_err(mds, "epoll wait failed %d\n", errno);
            continue;
        }
        ...
        if (ev.data.fd == hmo.conf.dcfd) {
            ...
            if (err < 0) {
                gk_err(mds, "epoll_ctl() add fd %d failed %d\n",
                         nfd, errno);
                continue;
            }
            gk_warning(mds, "Accept dconf connection %d\n", nfd);
        } else if (err) {
            if (ev.events & EPOLLERR || ev.events & EPOLLHUP) {
                gk_err(mds, "Hoo, the connection %d is broken\n",
                         ev.data.fd);
...
int dconf_init(void)
{
    ...
    if (hmo.conf.dcepfd == -1) {
        gk_err(mds, "epoll create failed %d\n", errno);
        err = errno;
        goto out;
    }
    err = fcntl(hmo.conf.dcfd, F_GETFL);
    if (err == -1) {
        gk_err(mds, "fcntl F_GETFL failed %d\n", errno);
        err = errno;
        goto out;
    }
    err = fcntl(hmo.conf.dcfd, F_SETFL, err | O_NONBLOCK);
    if (err == -1) {
        gk_err(mds, "fcntl F_SETFL failed %d\n", errno);
        err = errno;
        goto out;
    }
    ev.events = EPOLLIN | EPOLLPRI | EPOLLERR | EPOLLHUP | EPOLLET;
    ...
    if (err == -1) {
        gk_err(mds, "epoll add fd %d failed %d\n", hmo.conf.dcfd, errno);
        err = errno;
        goto out;
    }
    ...
    gk_info(mds, "create DCONF cmd channel: %s w/ %d\n", 
              hmo.conf.dcaddr, err);
out:
    return err;
}
